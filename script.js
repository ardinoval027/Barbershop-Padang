 ...
 function barbershopradius(){ //menampilkan barbershop berdasarkan radius
    ...
    console.log(pos.lat);
      console.log(pos.lng);
        ...
        klikInfoWindow(id);
              map.setZoom(13);
              $('#hasilcari').append("<tr><td>"+nama+"</td><td><a role='button' title='info' class='btn btn-default fa fa-info' onclick='detailbarbershop(\""+id+"\");info1();'></a></td><td><a role='button' title='booking' class='btn btn-default fa fa-envelope' value='Booking' onclick='booked(\""+id+"\")'></a></td></tr>");
            } 
            }    
          });
}
...
$('#hasilcari1').show();
  ...
  if (arrayLay==''){
    ...
    $.ajax({ url: server+'caritipesewa.php?tip='+arrayLay, data: "", dataType: 'json', success: function(rows){
      console.log(server+'caritipesewa.php?tip='+arrayLay);
      if(rows==null)
            {
              alert('Barbershop Tidak Ditemukan');
            }
...
function cari_barbershop1(rows) //fungsi cari barbershop berdasarkan nama
  {   
     $('#hasilcari1').show();
    $('#hasilcari').empty();
      hapusInfo();
...
function legenda()
{
  ...
  var layer = new google.maps.FusionTablesLayer(
    ...
    var legend = document.createElement('div');
        ...
        content.push('<p><div class="color a"></div>Barbershop</p>');

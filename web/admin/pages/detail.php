...
if(isset($_SESSION['A']) && $_SESSION['A']){
  $query = "SELECT barbershop.id, barbershop.nama as barbershop_name, barbershop.alamat, barbershop.image, 'Admin' as admin_name, 
  ST_X(ST_Centroid(geom)) As lng, ST_Y(ST_Centroid(geom)) As lat FROM barbershop where barbershop.id='$id_bbs'";  
} else {
...
while($row = pg_fetch_array($hasil)){
  ...
  <div class="col-sm-6"> 
				...
				<div class="btn-group">
						<a href="?page=updatebarbershopadmin&id=<?php echo $id_bbs ?>" class="btn btn-default" style ="background-color: #9e1134; color: white"><i class="fa fa-edit"></i>&nbsp Edit Information</a>
				</div>
                      </div>
	...
	<div class="col-sm-5"> <!-- menampilkan peta-->
                  <section class="panel">
                      ...
                      <div class="panel-body">
                         <?php $id=$_GET['id'] ?>
	<?php
	$querysearch="SELECT gallery_barbershop FROM barbershop_gallery where id='$id'";
	$hasil=pg_query($querysearch);
	 
	 while($baris = pg_fetch_array($hasil))
	 {
	 	
	 	// echo $baris['gallery_barbershop'];
	 	?>
	...
	<form role="form" action="act/uploadfotobbsadmin.php" enctype="multipart/form-data" method="post">
	  <div class="box-body">
